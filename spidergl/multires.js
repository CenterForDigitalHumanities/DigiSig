...
function createRtiViewer(idDiv, imageUrl, width, height, options) {
	width = parseFloat(width); // always expects pixels
	height = parseFloat(height); // always expects pixels
	DNShack = options.DNShack || '';
	...
	canvasContainer.empty(); // for reusability
	...
	$(aNode).css(options.linkNodeStyle || {
		width: "80px",
		height: "50px",
		position: "absolute",
		bottom: "0px",
	...
	aNode.innerText = "Thanks, Gianpaolo Palma";
	var linkNode = $("#" + options.linkNode) || canvasNode; //options
	linkNode.empty().append(aNode);

	if(!options.externalToolbar){
	...
	$(toolbar).css(options.toolbarStyle || {
		position: "absolute",
		top: "0px",
		left: "0px",
	...
	var toolbarNode = $("#" + options.toolbarNode) || canvasNode; //options
	toolbarNode.append(toolbar);

	$("#" + idDiv + "_div button").css("height", "");
	$("#" + idDiv + "_div button").css("width", "100%");
	}
	...
	divHelp.innerHTML = '<div id = "guideTable"><div id = "guideCell"> <div id = "guideList"><h3>WebRTIViewer<br/></h3><ul><li>Pan: LeftMouseButton + MouseMove.</li><li>Zoom in: MouseWhell or press the button <span><img src = "'+DNShack+'css/icons/zoomin.png" alt=""/></span></li><li>Zoom out: MouseWheel or press the button <span><img src = "'+DNShack+'css/icons/zoomout.png" alt=""/></span></li><li>To change the light direction: activate the light with the button <span><img src = "'+DNShack+'css/icons/light.png" alt=""/></span> and press LeftMouseButton + MouseMove or Ctrl + LeftMouseButton + MouseMove.</li><li>Fullscreen: press the button <span><img src = "'+DNShack+'css/icons/full.png" alt=""/></span> to active the fullscreen mode and the button <span><img src = "'+DNShack+'css/icons/full_on.png" alt=""/></span> to exit</li><li>To reset the viewpoint: press R.</li></ul><a href="http://vcg.isti.cnr.it/rti/webviewer.php" target = "_blank">Visual Computing Lab ISTI CNR Pisa</a></div></div></div>'
...
MultiRes.prototype = {
	...
	mouseMove: function (gl, x, y) {
		if (this.animating)
			return false;
		this._setLightDir(this.ui.mousePos.x, this.ui.mousePos.y); // cubap always light up
...
function MultiResRenderer(gl, cacheSizeInBytes, onLoad) {
	...
	// box mesh
	/******************************************************/
	var boxPositions = new Float32Array([-0.5, -0.5, 0.5,
	...
	if (false && this.lg) { // hiding the triangle/circle overlay
...
MultiResRenderer.prototype = {
	...
	_requestNodes: function () {
		...
		for (var i = 0;
			((i < k) && (requested < requestableCount)); ++i) {
			n = this._toRequest[i];
	...
	_setupTree: function () {
		...
		var quadPositions = new Float32Array([-0.5, 0.5, -0.5, -0.5,
			0.5, 0.5,
			0.5, -0.5
		]);
