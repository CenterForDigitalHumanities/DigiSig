    ...
    if('' != $column or null != $column)
    {
        ...
        If ($field != "all_fields") {
            If ($numberofresults > 0) {
                ...
                echo '<tr id="show_more_tr_'.$field.'" last_row_num='.$rowcount--.'><td colspan="3"><input type="button" class="digiBtn" id="show_more_btn_'.$field.'" value="Show More" offset=5 onclick=\'getNextData("'.$field.'", "'.$index.'", "'.$term.'", "'.$address.'", "'.$exact.'", '.$limit.')\' /><span id="load_next_pending_'.$field.'" style="display:none">Loading...</span></td></tr></tbody></table>';
